...
class UncertaintyLoss(Module):
    def __init__(self, input_slice=[21, 13, 17],
                 target_slice=[0, 1, 1], sub=0,
                 cuda_=True, mask=False):
    ...
    def forward(self, input, target):
        ...
        _, rooms_pred, icons_pred = pred_arr

        target_arr = torch.split(target, self.target_slice, 1)
        _, rooms_target, icons_target = target_arr
        ...
        rooms_target = rooms_target.type(torch.int64) - self.sub
        icons_target = icons_target.type(torch.int64) - self.sub
        # if self.cuda_:
